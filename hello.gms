...
eq9..	;
eq10..	sum(i, n(i)) =g= 2;
eq11..	sum(B, n(i)) =e= 2*ZB;
eq12..	sum(i, n(i)*b(i)) =g= 2*sum(i, n(i)-1);
eq13..	sum(i, n(i)*b(i)) =l= sum(i, n(i))*sum(i, n(i)-1);
eq14..  YSDx$(sum(SDx, n(i)) >= 1) =e= 1;
eq15..  YSDy$(sum(SDy, n(i)) >= 1) =e= 1;
eq16..  YSDz$(sum(SDz, n(i)) >= 1) =e= 1;
eq17a..	YSDx =l= sum(SDx, n(i));
eq17b..	sum(SDx, n(i)) =l= Nmax*YSDx*card(SDx);
eq18a..	YSDy =l= sum(SDx, n(i));
eq18b..	sum(SDy, n(i)) =l= Nmax*YSDy*card(SDy);
eq19a..	YSDz =l= sum(SDx, n(i));
eq19b..	sum(SDz, n(i)) =l= Nmax*YSDz*card(SDz);
...
eq27b..	sum(H, n(i)) =l= Nmax*YH*card(H);
eq28..	sum(O$(b(O) = 1), n(i)) =l= sum(H, n(i)*S(i)) + Nmax*(1-YH)*card(SDy);
eq29..	sum(O$(b(O) = 2), n(i)) =l= sum(H, n(i)*D(i)) + Nmax*(1-YH)*card(SDy);
eq30..	sum(O$(b(O) = 3), n(i)) =l= sum(H, n(i)*T(i)) + Nmax*(1-YH)*card(SDy);
